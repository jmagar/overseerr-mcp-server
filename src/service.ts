...
import type { OverseerrConfig } from './types.js';
import type { MediaRequest, Media, User, SearchResult, Stats, MovieResult, TvResult, RequestStatus, PageInfo } from './types.js';
...
export class OverseerrService {
  ...
  async searchAll(query: string, options: {
    ...
    return {
      movies: results.results.filter((r: { mediaType?: string }): r is MovieResult => r.mediaType === 'movie'),
      tvShows: results.results.filter((r: { mediaType?: string }): r is TvResult => r.mediaType === 'tv')
    };
  }
